...
namespace Khansho
{
    ...
    {
        private static readonly ushort[] s_CRCTable = new ushort[]
        {
        ...
        public static ushort CRC16_byte(this byte[] bytes, int offset = 0)
        {
            return bytes.CRC16_byte(offset, bytes.Length - offset);
        }

        public static ushort CRC16_byte(this byte[] bytes, int offset, int length)
        {
            int crc2 = 0xffff;
            for (int i = offset; i < length + offset; i++)
            {
                crc2 = (ushort)((crc2 >> 8) ^ s_CRCTable[(crc2 ^ bytes[i]) & 0xff]);
            }
            return (ushort)crc2;
        }

        public static ushort CheckSum(this byte[] bytes, int offset = 0)
        {
            return bytes.CheckSum(offset, bytes.Length - offset);
        }

        public static ushort CheckSum(this byte[] bytes, int offset, int length)
        {
            int sum = 0;

            while (length > 1)
            {
                sum += BitConverter.ToUInt16(bytes, offset);
                length -= 2;
                offset += 2;
            }
        ...
        public static short ToShort(this byte[] bytes, int start)
		{
        ...
        public static string ToStr(this byte[] bytes, int length)
		{
			if (bytes == null)
			{
				return string.Empty;
        ...
        {
            var packet = new Packet(bytes.Length);
            packet.CopyFrom(bytes);
            return packet;
        }

        public static void CopyFrom(this byte[] bytes, IntPtr ptr, int length)
        {
            Marshal.Copy(ptr, bytes, 0, length);
        }

        public static void CopyFrom(this byte[] src, byte[] bytes, int offset = 0)
        {
            GameUtils.BlockCopy(src, offset, bytes, 0, bytes.Length);
        }

        public static void CopyTo(this byte[] bytes, IntPtr ptr, int length)
        {
            Marshal.Copy(bytes, 0, ptr, length);
        }

        public static void CopyTo(this byte[] src, byte[] bytes, int offset = 0)
        {
            GameUtils.BlockCopy(bytes, offset, src, 0, src.Length);
        }
    }
}